...
namespace CityGeneration
{
    public static class MapCalculator
    {
        ...
        */
        ...
        public static Vector2 WorldToCell(Vector3 position)
        {
            var cell = Vector2.zero;
            var pos = Grid.Round(position);
            cell.x = pos.x / ConvertValue;
            cell.y = pos.z / ConvertValue;
            return cell;
        }
        
        public static int GetHeight(Vector3 cell)
        {
        ...
        static void GenerateCombination() // algorithm for generating all possible bitmasks -> use in case of losing foundation dictionary
        {
            for (var i = 0; i < (1 << 8); i++)
            {
                var isValid = true;
